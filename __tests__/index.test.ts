...
import { getMetadataArgsStorage, MetadataArgsStorage, RoutingControllersOptions } from 'routing-controllers'
...
describe('construct routes by passing classes for controllers', () => {
  ...
  runTestCases(storage, options, routes)
})

describe('construct routes by passing directories for controllers', () => {
  // Construct OpenAPI spec:
  const storage = getMetadataArgsStorage()
  const options = {
    controllers: [`${__dirname}/fixtures/controllers/**/*.ts`],
    routePrefix: '/api',
  }
  const routes = parseRoutes(storage, options)
...
function runTestCases(storage: MetadataArgsStorage, options: RoutingControllersOptions, routes: IRoute[]) {
  describe('index', () => {
